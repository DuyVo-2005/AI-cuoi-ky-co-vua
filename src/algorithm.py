...
def uninformed_search(root: SearchNode, type: str):
    ...
    while not open_list.is_empty():
        ...
        for action, new_state in generate_king_moves(n.state):
...
def DeepLimitedSearch(node: SearchNode, depth_limit):
    ...
    while stack:
        ...
        if len(path) < depth_limit:  #mở rộng nếu chưa đạt depth_limit
            for action, new_state in generate_king_moves(current_node.state):
                if not close_list.lookup(new_state):
...
#     global end_state_tuple
#     return abs(state[0] - end_state_tuple[0]) + abs(state[1] - end_state_tuple[1]) if (state[0] + state[1]) % 2 != 0 else abs(state[0] - end_state_tuple[0]) + abs(state[1] - end_state_tuple[1]) + 100# ưu tiên
...
def IDA_star(root: SearchNode):
    def search(node: SearchNode, path: set, threshold):# Tìm kiếm theo DFS với giới hạn threshold
        ...
        path.add(node.state)
        for action, new_state in generate_king_moves(node.state):
...
def simple_hill_climbing(root: SearchNode):
    ...
    while True:
        if is_goal(current_node.state):
            return extract_path(current_node)
        neighbors = generate_king_moves(current_node.state)
        if not neighbors:
            return None
...
def steepest_ascent_hill_climbing(root: SearchNode):
    ...
    while True:
        if is_goal(current_node.state):
            return extract_path(current_node)
        neighbors = generate_king_moves(current_node.state)#Trả về (action, state)
        if not neighbors:
            return None
...
def Beam_search(root: SearchNode):
    ...
    while not open_list.is_empty():
        ...
        if is_goal(current_node.state):
            visited_nodes = close_list
            return extract_path(current_node)
        neighbors, neighbors2 = generate_king_moves(current_node.state), None
        ...
        if len(open_list.deque) >= 2:
            ...
            neighbors2 = generate_king_moves(current_node2.state)
